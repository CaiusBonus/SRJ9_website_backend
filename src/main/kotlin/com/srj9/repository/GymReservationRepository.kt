...
interface GymReservationRepository: JpaRepository<GymReservation, Long> {
    ...
    @Query("SELECT r FROM GymReservation r join r.user WHERE r.date BETWEEN ?1 AND ?2 AND r.user LIKE ?3")
    fun findGymReservationsBetweenFirstDayAndLastDayAndUser(firstDay: Date, lastDay: Date, user: User): List<GymReservation>
